...
DB_PATH = os.path.join(_PROJECT_ROOT, "data", "genetics.db")
...
def init_db():
    ...
    # Table for Ancestry Panels (matches SQLAlchemy AncestryPanel model)
    cursor.execute('''
    CREATE TABLE IF NOT EXISTS ancestry_panels (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        population TEXT,
        rsid TEXT,
        gene TEXT,
    ...
    # Table for Guidance Data (matches SQLAlchemy Guidance model)
    cursor.execute('''
    CREATE TABLE IF NOT EXISTS guidance_data (
        ...
        medical TEXT,  -- JSON array
        screening TEXT  -- JSON array
    )
    ''')
...
def migrate_static_data():
        ...
        for rsid, info in data_dict.items():
            ...
            if 'interp' in info: # Recessive / Protective
                for genotype, interp in info['interp'].items():
                    ...
                    # The current static data structure is a bit simplified (just key=rsid). 
    ...
    for rsid, info in adverse_reaction_snps.items():
        ...
        for genotype, risk in info['interp'].items():
             ...
             INSERT INTO variant_interpretations (rsid, genotype, interpretation, condition, source)
             VALUES (?, ?, ?, ?, ?)
             ''', (rsid, genotype, risk, info.get('relevance'), 'Internal'))

    # Migrate Star Alleles
    for gene, alleles in star_allele_definitions.items():
        for allele, details in alleles.items():
            cursor.execute('''
            INSERT INTO star_alleles (gene, star_allele, function, description, haplotypes)
            VALUES (?, ?, ?, ?, ?)
            ''', (gene, allele, details['function'], details['description'], json.dumps(details['haplotypes'])))

    # Migrate CPIC Guidelines
    for gene, drugs in cpic_guidelines.items():
        for drug, phenotypes in drugs.items():
            for phenotype, recommendation in phenotypes.items():
                cursor.execute('''
                INSERT INTO cpic_guidelines (gene, drug, phenotype, recommendation)
                VALUES (?, ?, ?, ?)
                ''', (gene, drug, phenotype, recommendation))

    # Migrate PRS Models
    # Legacy models
    for condition, model in legacy_prs_models.items():
        cursor.execute('''
    ...
    for pop, snps in ancestry_panels_data.items():
        for rsid_val, details in snps.items():
            cursor.execute('''
            INSERT INTO ancestry_panels (population, rsid, gene, condition)
            VALUES (?, ?, ?, ?)
            ''', (pop, rsid_val, details['gene'], details['condition']))

    # Migrate Guidance Data
    guidance_data_static = {
        "Coronary Artery Disease": {
            "lifestyle": ["Adopt a Mediterranean diet", "150+ min moderate aerobic activity/week", "Maintain BMI 18.5-24.9", "Quit smoking"],
            "monitoring": ["Annual blood pressure checks", "Lipid panel every 4-6 years"],
            "medical": ["Discuss statin therapy if LDL elevated", "Low-dose aspirin if recommended"],
            "screening": ["Coronary calcium scan for risk refinement"],
        },
        "Type 2 Diabetes": {
            "lifestyle": ["Low-glycemic index diet", "5-7% weight loss if overweight", "Resistance training 2-3x/week"],
            "monitoring": ["Annual HbA1c testing", "Monitor blood pressure and cholesterol"],
            "medical": ["Metformin for pre-diabetes consideration"],
            "screening": ["Screening starting at age 35 for overweight adults"],
        },
        "Atrial Fibrillation": {
            "lifestyle": ["Limit alcohol consumption", "Manage stress and sleep apnea", "Maintain healthy weight"],
            "monitoring": ["Monitor pulse for irregularity", "Wearable device rhythm monitoring"],
            "medical": ["Anticoagulation based on CHA2DS2-VASc score"],
            "screening": ["Opportunistic screening in patients >65 years"],
        },
        "Colorectal Cancer": {
            "lifestyle": ["Limit red/processed meat", "Increase fiber intake", "Stay active"],
            "monitoring": ["Monitor for rectal bleeding or bowel changes"],
            "medical": ["Discuss aspirin chemoprevention"],
            "screening": ["Colonoscopy starting at age 45"],
        },
    }
    for condition, g_data in guidance_data_static.items():
        cursor.execute('''
        INSERT OR IGNORE INTO guidance_data (condition, lifestyle, monitoring, medical, screening)
        VALUES (?, ?, ?, ?, ?)
        ''', (condition, json.dumps(g_data['lifestyle']), json.dumps(g_data['monitoring']),
              json.dumps(g_data['medical']), json.dumps(g_data['screening'])))
...
# Initialize on module load if DB doesn't exist
