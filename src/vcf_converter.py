...
def convert_vcf_gz_to_tsv(input_vcf_gz_path, output_tsv_path):
    ...
    with gzip.open(input_vcf_gz_path, 'rt') as infile, \
         open(output_tsv_path, 'w', newline='') as outfile:
        
        writer = csv.writer(outfile, delimiter='\t')
        writer.writerow(['rsid', 'chromosome', 'position', 'CLNSIG']) # Write header for TSV
