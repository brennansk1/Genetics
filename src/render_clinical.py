...
def render_clinical_risk(dna_data):
    ...
    st.write(
        "This module will identify high-impact genetic variants with established clinical significance."
    )
    ...
    with st.expander("Click to see beginner-friendly definitions of genetic terms"):
        ...
        st.write(
            "**Carrier**: Someone who has one copy of a disease-causing variant but doesn't have symptoms."
        )
        st.write(
            "**Recessive**: A condition that requires two copies of a variant (one from each parent) to cause disease."
        )
    ...
    with col1:
        ...
        if clinvar_status == "healthy":
            ...
            st.warning(
                "⚠️ ClinVar API is currently unavailable - using local fallback data"
            )
        else:
    ...
    with col2:
        if st.button("🔄 Refresh API Status", key="refresh_clinvar_status"):
            with st.spinner("Checking API status..."):
                health_status = get_api_health_status()
                clinvar_status = health_status.get("clinvar", {}).get(
                    "status", "unknown"
                )
                st.rerun()
    ...
    if st.button("Run Pathogenic Variant Screener"):
                ...
                if use_live_data and clinvar_status == "healthy":
                    ...
                    if clinvar_results:
                        ...
                        for rsid, significance in clinvar_results.items():
                            if significance and (
                                "pathogenic" in significance.lower()
                                or "likely pathogenic" in significance.lower()
                            ):
                        ...
                        if pathogenic_variants:
                            st.warning(
                                "⚠️ Pathogenic or Likely Pathogenic variants found in your data:"
                            )
                            ...
                            **Clinical Interpretation:**
                            ...
                            st.success(
                                "✅ No pathogenic or likely pathogenic variants detected in ClinVar database."
                            )
                    else:
                        st.error(
                            "Failed to retrieve data from ClinVar API. Please try again or use local data."
                        )
                    ...
                    if clinvar_df is not None:
                        ...
                        if not merged_df.empty:
                            st.warning(
                                "⚠️ Pathogenic or Likely Pathogenic variants found in your data (Local Data):"
                            )
                            display_df = merged_df[
                                ["rsid", "chromosome", "position", "genotype", "CLNSIG"]
                            ].copy()
                            display_df["Data Source"] = "Local ClinVar Database"
                            st.dataframe(display_df)
                        else:
                            st.success(
                                "✅ No pathogenic or likely pathogenic variants from the local ClinVar database were found in your data."
                            )
                    else:
                        st.error(
                            "Local ClinVar database not available. Please check data files."
                        )
        ...
        st.write(
            "**Disease-Causing Variants**: Pathogenic variants are like 'broken' instructions in your DNA that can cause serious health conditions. This screening checks against a comprehensive database of known disease-causing changes."
        )
        st.write(
            "Most people don't carry pathogenic variants, but when they do, it can have significant implications for health and family planning."
        )
    ...
    if st.button("Run Recessive Carrier Status Report"):
        ...
        st.subheader("What Does This Mean?")
        st.write(
            "**Carrier Status**: Being a carrier means you have one copy of a disease-causing variant but are healthy yourself. For recessive conditions, you need two copies (one from each parent) to develop the disease."
        )
        st.write(
            "If both partners are carriers for the same condition, their children have a 25% chance of inheriting the disease."
        )
    ...
    if st.button("Run Hereditary Cancer Syndromes Analysis"):
        ...
        st.subheader("What Does This Mean?")
        st.write(
            "**Hereditary Cancer Risk**: Some genetic variants significantly increase cancer risk by affecting DNA repair or tumor suppression genes. Early detection and prevention strategies can be crucial."
        )
        st.write(
            "Positive results don't guarantee cancer development, but they inform screening and prevention plans."
        )
    ...
    if st.button("Run Protective Variant Highlights"):
        ...
        st.subheader("What Does This Mean?")
        st.write(
            "**Protective Variants**: Some genetic variants can actually protect against diseases or enhance health. These 'good' variants show how genetics can work in your favor."
        )
        st.write(
            "While protective variants are beneficial, they don't guarantee health - lifestyle still plays a major role."
        )
    ...
    selected_ancestry = st.selectbox(
        "Select your ancestry background for targeted screening",
        list(ancestry_panels.keys()),
    )
    ...
    if st.button("Run Ancestry-Aware Screening"):
        ...
        st.info(
            "**Note:** This screening is ancestry-informed but not definitive. Many conditions have variants not covered here. Consult a genetic counselor for comprehensive testing."
        )
    ...
    if st.button("Run ACMG Secondary Findings Screening"):
        ...
        for rsid, info in acmg_sf_variants.items():
            ...
            if not user_genotype.empty:
                ...
                if len(set(genotype)) > 1:
                    status = (
                        "ACMG secondary finding detected - Consult genetic counselor"
                    )
            results.append(
                {
                    "rsID": rsid,
        ...
        st.warning(
            "**ACMG Secondary Findings:** These are genes recommended by the American College of Medical Genetics for secondary findings analysis. Detection of variants in these genes may have significant medical implications. Professional genetic counseling is strongly recommended."
        )
