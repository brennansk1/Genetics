...
def analyze_clinvar_variants(df, clinvar_tsv_path):
    ...
    try:
        ...
        if not merged_df.empty:
            ...
            print(
                "No pathogenic or likely pathogenic variants from the ClinVar file were found in your data."
            )
            return pd.DataFrame()
    except FileNotFoundError:
        print(
            f"ClinVar TSV file not found at {clinvar_tsv_path}. Skipping this analysis."
        )
        return pd.DataFrame()
        ...
        if genotype in ["00", "--"]:
            ...
            if interpretation_map:
                interpretation = interpretation_map.get(
                    sorted_genotype, "Variant not standard."
                )
            else:
...
def process_dna_file(file_path, build, liftover_chain_path=None):
    ...
    print(
        f"DEBUG: Found header at line {data_start_line}: {lines[data_start_line] if data_start_line < len(lines) else 'NOT FOUND'}"
    )
    ...
    print(
        f"DEBUG: First few rsid values: {df['rsid'].head().tolist() if 'rsid' in df.columns else 'NO RSID COLUMN'}"
    )
    ...
    print(
        f"DEBUG: rsid values sample: {df['rsid'].head().tolist() if 'rsid' in df.columns else 'NO RSID'}"
    )
    ...
    if build.upper() == "GRCH38":
        if not liftover_chain_path or not os.path.exists(liftover_chain_path):
            raise FileNotFoundError(
                "GRCh38 build specified, but a valid liftover chain file was not provided or found."
            )
...
def analyze_high_impact_risks(df):
    ...
    if rs429358_geno != "Not in data" and rs7412_geno != "Not in data":
        if (
            "T" in rs429358_geno
            and "T" in rs7412_geno
            and len(set(rs429358_geno)) == 1
            and len(set(rs7412_geno)) == 1
        ):
            apoe_status = "E2/E2 (Lower Risk)"
        elif "T" in rs429358_geno and "C" in rs7412_geno:
            apoe_status = "E2/E3"
        elif "C" in rs429358_geno and "T" in rs7412_geno:
            apoe_status = "E3/E4 (Increased Risk)"
        elif (
            "C" in rs429358_geno
            and "C" in rs7412_geno
            and len(set(rs429358_geno)) == 1
            and len(set(rs7412_geno)) == 1
        ):
            apoe_status = "E4/E4 (Significantly Increased Risk)"
        elif ("C" in rs429358_geno and "T" in rs429358_geno) and ("T" in rs7412_geno):
            apoe_status = "E2/E4 (Increased Risk)"
        else:
            apoe_status = "E3/E3 (Typical Risk)"
    ...
    high_impact_snps = {
        "Hereditary Cancer Syndromes": {
            "rs80357906": {
                "gene": "BRCA1",
                "risk": "Hereditary Cancer Risk (185delAG)",
            },
            "rs80357713": {
                "gene": "BRCA1",
                "risk": "Hereditary Cancer Risk (5382insC)",
            },
            "rs80359551": {
                "gene": "BRCA2",
                "risk": "Hereditary Cancer Risk (6174delT)",
            },
            "rs63750247": {
                "gene": "MSH2",
                "risk": "Lynch Syndrome (Colorectal Cancer Risk)",
            },
            ...
            "rs1042522": {
                "gene": "TP53",
                "risk": "Li-Fraumeni Syndrome (Common Polymorphism)",
            },
            "rs121964876": {
                "gene": "CDH1",
                "risk": "Hereditary Diffuse Gastric Cancer",
            },
            "rs74799832": {
                "gene": "RET",
                "risk": "Multiple Endocrine Neoplasia Type 2",
            },
        ...
        "Cardiovascular Conditions": {
            "rs6025": {
                "gene": "F5",
                "risk": "Factor V Leiden (Hereditary Clotting Risk)",
            },
            "rs1799963": {
                "gene": "F2",
                "risk": "Prothrombin G20210A (Hereditary Clotting Risk)",
            },
        ...
        "Autoimmune & Inflammatory Conditions": {
            ...
            "rs2476601": {
                "gene": "PTPN22",
                "risk": "Rheumatoid Arthritis & Autoimmunity",
            },
        ...
        "Other Major Diseases": {
            ...
            "rs1061170": {
                "gene": "CFH",
                "risk": "Age-Related Macular Degeneration (AMD)",
            },
            "rs28929474": {
                "gene": "SERPINA1",
                "risk": "Alpha-1 Antitrypsin Deficiency (PiZ allele)",
            },
        },
    }
        ...
        for rsid, info in snps.items():
            ...
            if genotype != "Not in data" and len(set(list(genotype))) > 1:
                result = {
                    ...
                    "Associated Risk": info["risk"],
                    "Genotype": genotype,
                    "Interpretation": "Risk variant detected. CONSULT A GENETIC COUNSELOR.",
                }
...
def analyze_pgx_and_wellness(df):
    ...
    wellness_snps = {
        "rs762551": {
            ...
            "interp": {"AA": "Fast", "AC": "Slow", "CC": "Slow"},
        },
        "rs4988235": {
            ...
            "interp": {
                "GG": "Likely Tolerant",
                "AG": "Likely Tolerant",
                "AA": "Likely Intolerant",
            },
        },
        "rs1815739": {
            "trait": "Muscle Performance (ACTN3)",
            "interp": {
                "CC": "Power/Sprint Profile",
                "CT": "Mixed Profile",
                "TT": "Endurance Profile",
            },
        },
        "rs1801133": {
            "gene": "MTHFR C677T",
            "trait": "Folate Processing",
            "interp": {"GG": "Normal", "AG": "Reduced", "AA": "Significantly Reduced"},
        },
        "rs174547": {
            "gene": "FADS1",
            "trait": "Fatty Acid Processing (Omega-3/6)",
            "interp": {"CC": "Normal conversion", "CT": "Reduced", "TT": "Reduced"},
        },
        "rs2282679": {
            "gene": "GC",
            ...
            "interp": {
                "AA": "Typical levels",
                "AG": "Predisposition to lower levels",
                "GG": "Predisposition to lower levels",
            },
        },
    }
...
def analyze_prs(df, results_dir):
    ...
    def calculate_prs_and_plot(model_data, condition_name):
        ...
        if merged_df.empty:
            print(
                f"No overlapping SNPs found for {condition_name} PRS model. Skipping."
            )
            return
        ...
        population_scores = np.random.normal(
            loc=np.mean(prs_model_df.effect_weight),
            scale=np.std(prs_model_df.effect_weight) * 2.5,
            size=10000,
        )
        percentile = (
            np.sum(population_scores < user_prs) / len(population_scores)
        ) * 100
        ...
        fig.add_vline(
            ...
            line_dash="dash",
            line_color="red",
            annotation_text=f"Your Score: {user_prs:.3f}",
            annotation_position="top right",
        )
        fig.update_layout(
            title_text=f"Polygenic Risk Score for {condition_name}<br>Your score is at the {percentile:.1f}th percentile."
        )

        output_path = os.path.join(
            results_dir, f"prs_{condition_name.replace(' ', '_')}.html"
        )
    ...
    print(
        "  > Guidance: Manage blood pressure, cholesterol, maintain a healthy weight, and avoid smoking."
    )
    calculate_prs_and_plot(t2d_model, "Type 2 Diabetes")
    print(
        "  > Guidance: Maintain a healthy diet, engage in regular physical activity, and manage weight."
    )
    calculate_prs_and_plot(afib_model, "Atrial Fibrillation")
    print(
        "  > Guidance: Healthy lifestyle, managing blood pressure, and avoiding excessive alcohol are key preventative measures."
    )
    calculate_prs_and_plot(stroke_model, "Ischemic Stroke")
    print(
        "  > Guidance: Controlling blood pressure and cholesterol are the most critical preventative steps."
    )
    calculate_prs_and_plot(crc_model, "Colorectal Cancer")
    print(
        "  > Guidance: Adherence to screening guidelines (e.g., colonoscopy) is critical."
    )
...
def generate_pdf_report(report_data, results_dir):
    ...
    if "high_impact" in report_data and not report_data["high_impact"].empty:
        ...
        data = [report_data["high_impact"].columns.tolist()] + report_data[
            "high_impact"
        ].values.tolist()
        table = Table(data)
        table.setStyle(
    ...
    if "pgx" in report_data and not report_data["pgx"].empty:
        ...
        data = [report_data["pgx"].columns.tolist()] + report_data[
            "pgx"
        ].values.tolist()
        table = Table(data)
        table.setStyle(
    ...
    if "wellness" in report_data and not report_data["wellness"].empty:
        ...
        data = [report_data["wellness"].columns.tolist()] + report_data[
            "wellness"
        ].values.tolist()
        table = Table(data)
        table.setStyle(
...
def main():
    ...
    print(
        "This script provides health insights far beyond standard AncestryDNA reports."
    )
    print(
        "It analyzes clinical risks, medication responses, complex diseases, and wellness traits."
    )
    ...
    if not os.path.exists(clinvar_tsv_path):
        print(
            f"'{clinvar_tsv_path}' not found. Converting '{clinvar_vcf_path}' to TSV..."
        )
        try:
            convert_vcf_gz_to_tsv(clinvar_vcf_path, clinvar_tsv_path)
        except FileNotFoundError:
            print(
                f"ERROR: '{clinvar_vcf_path}' not found. Please download it and place it in the same directory."
            )
            return
        except Exception as e:
    ...
    try:
        ...
        if CONFIG["performance"]["enable_parallel_processing"]:
            # Use parallel processing
            from .parallel_utils import run_parallel_analyses

            analysis_results = run_parallel_analyses(user_df, results_dir)

            report_data["high_impact"] = analysis_results["high_impact"]
            report_data["clinvar"] = analysis_results["clinvar"]
            report_data["carrier_status"] = analysis_results["carrier_status"]
            report_data["protective_variants"] = analysis_results["protective_variants"]
            report_data["pgx"] = analysis_results["pgx"]
            report_data["wellness"] = analysis_results["wellness"]
            # PRS generates files directly, no return value
        else:
