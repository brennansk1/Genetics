...
class GenomeWidePRS:
    ...
    def _calculate_prs_score_gpu(
        ...
        try:
            ...
            # Use CuPy if available (preferred for numerical computations)
            if CUPY_AVAILABLE:
                allele_counts_gpu = cp.array(allele_counts, dtype=cp.float32)
                weights_gpu = cp.array(weights, dtype=cp.float32)
                prs_score = float(cp.sum(allele_counts_gpu * weights_gpu))
            elif TORCH_AVAILABLE:
                device = CONFIG["gpu"]["device"] if torch.cuda.is_available() else "cpu"
                allele_counts_gpu = torch.tensor(
                    allele_counts, dtype=torch.float32, device=device
                )
                weights_gpu = torch.tensor(weights, dtype=torch.float32, device=device)
                prs_score = float(torch.sum(allele_counts_gpu * weights_gpu).cpu())
            else:
                # Fallback to CPU if GPU libraries not available
                prs_score = sum(a * w for a, w in zip(allele_counts, weights))

            return prs_score, len(allele_counts), total_snps

        except Exception as e:
            print(f"GPU PRS calculation failed: {e}. Falling back to CPU.")
            # Fallback to CPU calculation
            return GenomeWidePRS.calculate_prs_score(
                snp_data, effect_weights, effect_alleles
            )
    ...
    def calculate_percentile(
        ...
        population_scores = np.random.normal(
            population_mean, population_std, population_size
        )
    ...
    @staticmethod
    def normalize_prs_score(
        raw_score: float, model_mean: float, model_std: float
    ) -> float:
        """
    ...
    @staticmethod
    def calculate_ancestry_adjusted_percentile(
        user_score: float,
        population_mean: float,
        population_std: float,
        ...
        np.random.seed(42)  # For reproducibility
        population_scores = np.random.normal(
            adjusted_mean, adjusted_std, population_size
        )
    ...
    @staticmethod
    def calculate_ancestry_adjusted_prs_score(
        snp_data: pd.DataFrame,
        effect_weights: Dict[str, float],
        effect_alleles: Dict[str, str],
        inferred_ancestry: str,
    ...
    def calculate_genomewide_prs(
        ...
        use_ancestry_adjustment: bool = False,
    ) -> Dict:
        """
        ...
        if use_ancestry_adjustment and ancestry_result:
            result.update(
                {
                    "inferred_ancestry": ancestry_result.get(
                        "primary_ancestry", "Unknown"
                    ),
                    "ancestry_confidence": ancestry_result.get("confidence", 0.0),
                    "admixture_proportions": ancestry_result.get(
    ...
    def batch_calculate_prs(
        ...
        for i, model in enumerate(models):
            if progress_callback:
                progress_callback(
                    f"Processing model {i+1}/{len(models)}: {model.get('pgs_id', 'Unknown')}"
                )
    ...
    def calculate_simple_prs(snp_data: pd.DataFrame, model: Dict) -> Dict:
        ...
        return {
            ...
            "trait": model.get("trait", "Unknown"),
            "prs_score": prs_score,
            "normalized_score": calculator.normalize_prs_score(
                prs_score, population_mean, population_std
            ),
            "percentile": percentile,
            "snps_used": snps_used,
    ...
    def validate_prs_calculation(
        ...
        if validation_results["coverage_percentage"] < 0.5:
            validation_results["warnings"].append(
                f"Low SNP coverage: {validation_results['coverage_percentage']:.1f}"
            )

        if validation_results["coverage_percentage"] < 0.1:
            validation_results["errors"].append(
                "Extremely low SNP coverage - results may be unreliable"
            )
    ...
    def validate_ancestry_adjusted_prs(
        self, snp_data: pd.DataFrame, model: Dict, ancestry_result: Dict
    ) -> Dict:
        """
        ...
        if validation["ancestry_confidence"] < 0.5:
            validation["warnings"].append(
                "Low confidence in ancestry inference - results may be less reliable"
            )
        ...
        if "effect_weights" in model:
            ...
            validation["ancestry_prs_overlap_percentage"] = (
                overlap / len(ancestry_rsids) if ancestry_rsids else 0
            )

            if overlap == 0:
                validation["warnings"].append(
                    "No overlap between ancestry-informative SNPs and PRS model SNPs"
                )

        # Overall recommendation
        if (
            validation["ancestry_confidence"] >= 0.7
            and validation["coverage_percentage"] >= 50
        ):
            validation["recommendation"] = "Ancestry-adjusted PRS recommended"
        elif validation["ancestry_confidence"] >= 0.5:
            validation["recommendation"] = (
                "Ancestry adjustment may provide moderate improvement"
            )
        else:
            validation["recommendation"] = (
                "Consider manual ancestry specification or skip ancestry adjustment"
            )

        return validation

    @staticmethod
    def compare_adjusted_vs_unadjusted(
        snp_data: pd.DataFrame, pgs_id: str, cache_dir: str = "cache/prs"
    ) -> Dict:
        """
        ...
        comparison = {
            ...
            "differences": {
                "prs_score_diff": adjusted_result.get("prs_score", 0)
                - unadjusted_result.get("prs_score", 0),
                "percentile_diff": adjusted_result.get("percentile", 0)
                - unadjusted_result.get("percentile", 0),
                "normalized_score_diff": adjusted_result.get("normalized_score", 0)
                - unadjusted_result.get("normalized_score", 0),
            },
            "adjustment_applied": adjusted_result.get(
                "ancestry_adjustment_used", False
            ),
        ...
        if unadjusted_result.get("prs_score", 0) != 0:
            comparison["differences"]["prs_score_relative"] = (
                comparison["differences"]["prs_score_diff"]
                / abs(unadjusted_result["prs_score"])
                * 100
            )
