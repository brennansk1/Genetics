...
class LifetimeRiskCalculator:
    ...
    def _create_default_risk_data(self):
        ...
        conditions = [
            ...
            'coronary_artery_disease', 'type_2_diabetes', 'alzheimers_disease'
        ]
        ...
        base_rates = {
            'breast_cancer': {'female': [25, 50, 100, 200, 350, 500, 600, 550, 400, 250, 150, 80, 40, 20, 10, 5],
                             'male': [0.5, 1, 2, 3, 4, 5, 6, 5, 4, 3, 2, 1, 0.5, 0.2, 0.1, 0.05]},
            'prostate_cancer': {'male': [10, 20, 50, 150, 300, 600, 1000, 1200, 1000, 800, 600, 400, 200, 100, 50, 20],
                               'female': [0.1, 0.2, 0.5, 1, 1.5, 2, 2.5, 2, 1.5, 1, 0.5, 0.2, 0.1, 0.05, 0.02, 0.01]},
            'colorectal_cancer': {'female': [5, 10, 20, 40, 80, 120, 150, 140, 120, 100, 80, 60, 40, 25, 15, 8],
                                 'male': [8, 15, 30, 60, 120, 180, 220, 200, 170, 140, 110, 80, 50, 30, 18, 10]},
            'coronary_artery_disease': {'female': [20, 30, 50, 100, 200, 400, 600, 700, 600, 500, 400, 300, 200, 100, 50, 20],
                                       'male': [50, 80, 150, 300, 600, 1000, 1400, 1500, 1300, 1100, 900, 700, 500, 300, 150, 50]},
            'type_2_diabetes': {'female': [50, 80, 120, 200, 350, 500, 600, 550, 450, 350, 250, 150, 80, 40, 20, 10],
                               'male': [60, 100, 150, 250, 450, 650, 800, 750, 600, 450, 300, 180, 100, 50, 25, 12]},
            'alzheimers_disease': {'female': [10, 15, 25, 50, 100, 200, 400, 600, 700, 600, 400, 200, 100, 50, 25, 10],
                                  'male': [8, 12, 20, 40, 80, 150, 300, 450, 550, 500, 350, 200, 100, 50, 25, 10]}
        }
            ...
            for i, age in enumerate(ages):
                for sex in ['male', 'female']:
                    if sex in base_rates[condition]:
                        rate = base_rates[condition][sex][i] / 100000  # Convert to proportion
                        risk_data.append({
                            'condition': condition,
                            'sex': sex,
                            'age_start': age,
    ...
    def _load_life_expectancy_data(self):
        ...
        life_expectancy = {
            'male': {20: 58.2, 25: 53.8, 30: 49.4, 35: 45.0, 40: 40.7, 45: 36.4,
                    50: 32.2, 55: 28.1, 60: 24.1, 65: 20.3, 70: 16.6, 75: 13.1,
                    80: 9.9, 85: 7.1, 90: 4.9, 95: 3.2, 100: 2.0},
            'female': {20: 61.8, 25: 57.2, 30: 52.6, 35: 48.0, 40: 43.5, 45: 39.0,
                      50: 34.6, 55: 30.3, 60: 26.1, 65: 22.1, 70: 18.2, 75: 14.4,
                      80: 11.0, 85: 8.0, 90: 5.6, 95: 3.7, 100: 2.3}
        }
        self.life_expectancy_data = life_expectancy

    def calculate_lifetime_risk(self,
                               condition: str,
                               current_age: int,
                               sex: str,
                               ...
                               competing_risks: bool = True) -> Dict:
        """
        ...
        try:
            ...
            if condition_data.empty:
                return {'success': False, 'error': f'No data available for {condition} in {sex}s'}
            ...
            condition_data['adjusted_rate'] = condition_data['incidence_rate'] * total_modifier
            ...
            scenarios = self._calculate_risk_scenarios(
                condition_data, current_age, competing_risks,
                prs_percentile, ancestry, lifestyle_modifier
            )
    ...
    def _calculate_risk_trajectory(self,
                                 condition_data: pd.DataFrame,
                                 current_age: int,
                                 competing_risks: bool) -> pd.DataFrame:
        ...
        if future_data.empty:
            ...
            future_data['age_start'] = future_data['age_start'] + (current_age - future_data['age_start'].min())
        ...
        if competing_risks:
            survival_probs = self._calculate_survival_probabilities(ages, future_data['sex'].iloc[0])
        ...
        trajectory = pd.DataFrame({
            ...
            'survival_probability': survival_probs if competing_risks else np.ones_like(rates)
        })

        return trajectory

    def _calculate_survival_probabilities(self, ages: np.ndarray, sex: str) -> np.ndarray:
    ...
    def _calculate_confidence_intervals(self,
                                      ...
                                      condition_data: pd.DataFrame) -> Dict:
    ...
    def get_risk_summary(self, results: Dict) -> Dict:
        ...
        if not results.get('success', False):
            return {'summary': 'Unable to calculate lifetime risk', 'details': results.get('error', 'Unknown error')}
        ...
        if lifetime_risk < 0.05:
            risk_level = "Very Low"
            interpretation = f"Your lifetime risk of {condition.replace('_', ' ')} is below 5%."
        elif lifetime_risk < 0.10:
            risk_level = "Low"
            interpretation = f"Your lifetime risk of {condition.replace('_', ' ')} is between 5-10%."
            ...
            risk_level = "High"
            interpretation = f"Your lifetime risk of {condition.replace('_', ' ')} is above 30%."
...
def calculate_lifetime_risk(condition: str,
                           current_age: int,
                           sex: str,
                           prs_percentile: float = 50.0,
                           ancestry: str = 'European',
                           lifestyle_modifier: float = 1.0) -> Dict:
