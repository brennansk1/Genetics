...
def render_pharmacogenomics(dna_data):
    ...
    st.write(
        "This module provides comprehensive insights into potential responses to common medications using both basic SNP analysis and advanced star allele calling."
    )
    ...
    with st.expander("Click to see beginner-friendly definitions of genetic terms"):
        st.write(
            "**Star Allele**: A specific combination of genetic variants in a gene that affects how your body processes medications."
        )
        st.write(
            "**Metabolizer Status**: How quickly or slowly your body breaks down medications (like digestion speed for drugs)."
        )
        st.write(
            "**Haplotype**: A set of DNA variations that tend to be inherited together on the same chromosome."
        )
        st.write(
            "**CNV (Copy Number Variation)**: Differences in the number of copies of a DNA segment between individuals."
        )
        st.write(
    ...
    st.write(
        "Advanced analysis using CPIC-compliant star allele calling for accurate pharmacogenomic predictions."
    )
    ...
    if st.button("Run Star Allele Analysis"):
        ...
        for gene in star_genes:
            try:
                ...
                if "error" not in allele_result:
                    ...
                    metabolizer_display = (
                        f"{allele_result['metabolizer_status']}{warning_indicator}"
                    )
        ...
        if star_results:
            ...
            for result in star_results:
                if (
                    result["Metabolizer Status"] != "Unknown"
                    and result["Metabolizer Status"] != "Error"
                ):
                    with st.expander(
                        f"{result['Gene']} - {result['Metabolizer Status']}"
                    ):
                        gene = result["Gene"]
            ...
            st.write(
                "**Metabolizer Status as Digestion Speed**: Think of your metabolizer status like how fast your body 'digests' medications. Poor metabolizers are like slow digesters who need smaller doses, while ultra-rapid metabolizers are like fast digesters who might need higher doses to get the same effect."
            )
            st.write(
                "Star alleles are specific combinations of genetic variants that determine how efficiently your liver processes medications. This affects drug effectiveness and side effect risk."
            )
    ...
    if st.button("Run Basic SNP Analysis"):
        with st.spinner("Analyzing pharmacogenomic variants..."):
            ...
            if use_live_data and pharmgkb_status == "healthy":
                try:
                    live_pharmgkb_data = get_pharmgkb_data(
                        rsids_to_check, use_cache=True
                    )
                except Exception as e:
            ...
            if live_pharmgkb_data:
                st.info(
                    f"✅ Integrated data from {len([r for r in results if 'Live PharmGKB' in r['Data Sources']])} PharmGKB variants"
                )

            # Educational content for basic SNP analysis
            st.subheader("What Does This Mean?")
            st.write(
                "**Individual Genetic Variants**: Each rsID represents a specific location in your DNA where variation can occur. These single changes can affect how your body responds to medications by altering enzyme function."
            )
            st.write(
                "This analysis looks at individual genetic markers that influence drug metabolism, similar to checking individual ingredients in a recipe."
            )
    ...
    if st.button("Run Adverse Drug Reaction Sensitivity Analysis"):
        ...
        st.subheader("What Does This Mean?")
        st.write(
            "**Genetic Risk for Side Effects**: Some genetic variants can increase your risk of experiencing adverse reactions to certain medications. This is like having a sensitivity to specific ingredients that can cause allergic reactions."
        )
        st.write(
            "Understanding these variants helps healthcare providers choose safer medication options and monitor for potential side effects."
        )
