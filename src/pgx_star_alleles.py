...
class StarAlleleCaller:
    ...
    def call_star_alleles(self, gene: str, genotype_data: pd.DataFrame) -> Dict[str, str]:
        """
        ...
        try:
            ...
            haplotype_completeness = self._check_haplotype_completeness(alleles, user_haplotypes)

            # Determine metabolizer status
            metabolizer_status = self._determine_metabolizer_status(allele1, allele2, alleles)
            ...
            if haplotype_completeness < 0.5:
                result['warning'] = 'Incomplete haplotype data - results may be less accurate'
            ...
            if rsid in genotype_data.index:
                ...
                if genotype and len(genotype) == 2:
                    ...
                    if genotype[0] == genotype[1]:
                        ...
                        haplotypes.extend([f"{rsid}:{genotype[0]}", f"{rsid}:{genotype[0]}"])
                    else:
                        # Heterozygous - different alleles
                        haplotypes.extend([f"{rsid}:{genotype[0]}", f"{rsid}:{genotype[1]}"])
                elif genotype:
                    # Single allele or other format
                    haplotypes.append(f"{rsid}:{genotype}")
        return haplotypes

    def _determine_alleles(self, alleles: Dict, user_haplotypes: List[str]) -> Tuple[str, str]:
        ...
        for allele_name, allele_info in alleles.items():
            ...
            if not required_haps:  # Reference allele *1
                ...
                if all(hap in user_haplotypes for hap in required_haps):
                    ...
                    allele_counts[allele_name] = user_haplotypes.count(list(required_haps)[0]) if required_haps else 1
        ...
        for allele_name, count in sorted_alleles:
            ...
            if allele_haps:
                ...
                allele_hap_count = min(remaining_haplotypes.count(hap) for hap in allele_haps) if allele_haps else 0
        ...
        if len(sorted_alleles) == 2 and sorted_alleles[0] == sorted_alleles[1] and sorted_alleles[0] != '*1':
            return sorted_alleles[0], sorted_alleles[0]

        return sorted_alleles[0], sorted_alleles[1]

    def _determine_metabolizer_status(self, allele1: str, allele2: str, alleles: Dict) -> str:
    ...
    def _get_function_description(self, allele1: str, allele2: str, alleles: Dict) -> str:
        """Get combined function description"""
        func1 = alleles.get(allele1, {}).get('function', 'Normal')
        func2 = alleles.get(allele2, {}).get('function', 'Normal')
        return f"{func1}/{func2}"

    def _check_haplotype_completeness(self, alleles: Dict, user_haplotypes: List[str]) -> float:
        ...
        completeness = len(found_snps) / len(all_required_snps) if all_required_snps else 1.0

        return completeness

    def get_cpic_recommendations(self, gene: str, metabolizer_status: str, drug: str = None) -> Dict[str, str]:
        ...
        if drug and drug in gene_guidelines:
            return {drug: gene_guidelines[drug].get(metabolizer_status, 'No specific recommendation')}
        else:
            # Return all drugs for this gene
            recommendations = {}
            for drug_name, statuses in gene_guidelines.items():
                recommendations[drug_name] = statuses.get(metabolizer_status, 'No specific recommendation')
