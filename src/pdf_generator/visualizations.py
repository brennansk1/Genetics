...
def generate_metabolism_funnel(drug_name, metabolizer_status, gene_name):
    ...
    status_configs = {
        'Ultra-Rapid': {'widths': [0.8, 0.9, 0.95], 'color': '#FF6B6B', 'description': 'Too fast conversion'},
        'Rapid': {'widths': [0.6, 0.75, 0.85], 'color': '#FFA500', 'description': 'Fast conversion'},
        'Normal': {'widths': [0.4, 0.55, 0.7], 'color': '#4CAF50', 'description': 'Normal conversion'},
        'Intermediate': {'widths': [0.3, 0.4, 0.5], 'color': '#FFC107', 'description': 'Slow conversion'},
        'Poor': {'widths': [0.2, 0.25, 0.3], 'color': '#F44336', 'description': 'Very slow conversion'}
    }
    ...
    for i, (level, y) in enumerate(zip(levels, y_positions)):
        ...
        if i < len(levels) - 1:  # Not the last level
            ...
            ax.fill([x_left, x_right, next_x_right, next_x_left],
                    [y, y, y+1, y+1], color=config['color'], alpha=0.7)
        else:
            # Draw rectangle for last level
            ax.fill([x_left, x_right, x_right, x_left],
                    [y, y, y+0.8, y+0.8], color=config['color'], alpha=0.7)

        # Add level labels
        ax.text(0.5, y + 0.4, level, ha='center', va='center', fontsize=9, fontweight='bold')

    # Add arrows showing flow
    for i in range(len(levels)-1):
        ax.arrow(0.5, y_positions[i] + 0.8, 0, 0.2, head_width=0.05, head_length=0.05,
