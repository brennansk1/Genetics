...
try:
    ...
    from run_analysis import (
        process_dna_file, analyze_high_impact_risks, analyze_pgx_and_wellness,
        analyze_prs, analyze_clinvar_variants, analyze_recessive_carrier_status,
        analyze_protective_variants, analyze_expanded_carrier_status
    )
...
class AnalysisDebugger:
    ...
    def __init__(self, dna_file_path: str, genome_build: str = 'GRCh37'):
        ...
        logger.info(f"Initializing debugger for {dna_file_path} (build: {genome_build})")
    ...
    def test_ancestry_inference(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            if 'rsid' not in df.columns and df.index.name != 'rsid':
                logger.error(f"DataFrame missing 'rsid' column/index. Available columns: {list(df.columns)}, Index name: {df.index.name}")
                return {'success': False, 'error': 'Missing rsid column/index in SNP data'}
            ...
            if 'rsid' not in ancestry_infer.aims_data.columns:
                logger.error(f"AIMs data missing 'rsid' column. Available columns: {list(ancestry_infer.aims_data.columns)}")
            ...
            if ancestry_result['success']:
                self.log_success("Ancestry Inference",
                ...
                try:
                    ...
                    adjustment_params = get_ancestry_adjusted_prs_params(ancestry_result.get('primary_ancestry', 'European'))
                    self.log_success("Ancestry Adjustment", f"Generated adjustment parameters for {ancestry_result.get('primary_ancestry', 'Unknown')}")
                except Exception as e:
                    self.log_warning("Ancestry Adjustment", f"Failed to generate adjustment parameters: {e}")

                # Test ancestry validation
                try:
                    validation = ancestry_infer.validate_ancestry_inference(df, ancestry_result)
                    self.log_success("Ancestry Validation", f"Coverage: {validation.get('coverage_percentage', 0):.1f}%, Confidence: {validation.get('confidence_assessment', 'Unknown')}")
                except Exception as e:
                    self.log_warning("Ancestry Validation", f"Failed to validate ancestry: {e}")

                # Test different inference methods
                try:
                    pca_result = ancestry_infer.infer_ancestry(df, method='pca')
                    if pca_result['success']:
                        self.log_success("Ancestry PCA Method", f"PCA result: {pca_result.get('primary_ancestry', 'Unknown')}")
                    else:
                        self.log_warning("Ancestry PCA Method", f"PCA failed: {pca_result.get('error', 'Unknown')}")
                except Exception as e:
                    self.log_warning("Ancestry PCA Method", f"PCA method not available: {e}")

            else:
                self.log_warning("Ancestry Inference", f"Failed: {ancestry_result.get('error', 'Unknown error')}")
    ...
    def test_high_impact_risks(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if not risks_df.empty:
                self.log_success("High Impact Risks", f"Found {len(risks_df)} risk variants")
            else:
    ...
    def test_pgx_wellness(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        try:
            ...
            self.log_success("Wellness Analysis", f"Analyzed {len(wellness_df)} wellness variants")
            ...
            try:
                ...
                self.log_success("Individual Wellness SNPs", f"Found {found_snps}/{len(wellness_results)} wellness SNPs in data")
            except Exception as e:
                self.log_warning("Individual Wellness SNPs", f"Failed to analyze individual SNPs: {e}")
            ...
            for category, snps in wellness_categories.items():
                ...
                self.log_success(f"Wellness {category}", f"Found {found_in_category}/{len(snps)} SNPs in {category.lower()} category")
        ...
        try:
            ...
            try:
                ...
                if simple_result['success']:
                    self.log_success("Simple PRS", f"Score: {simple_result.get('prs_score', 0):.3f}")
                else:
                    self.log_warning("Simple PRS", f"Failed: {simple_result.get('error', 'Unknown')}")
            except Exception as e:
            ...
            try:
                ...
                self.log_success("Percentile Calculation", f"Score {test_score} = {percentile:.1f}th percentile")
            except Exception as e:
            ...
            try:
                ...
                self.log_success("Ancestry-Adjusted Percentile", f"European adjustment: {adjusted_percentile:.1f}th percentile")
            except Exception as e:
            ...
            for pgs_id in test_pgs_ids:
                try:
                    result = calculator.calculate_genomewide_prs(df, pgs_id, use_ancestry_adjustment=True)
                    if result['success']:
                        self.log_success(f"PRS {pgs_id}",
                        ...
                        self.log_warning(f"PRS {pgs_id}", f"Failed: {result.get('error', 'Unknown error')}")
                    prs_results.append(result)
                except Exception as e:
                    self.log_error(f"PRS {pgs_id}", e, f"PRS calculation failed for {pgs_id}")
                    prs_results.append({'success': False, 'error': str(e), 'pgs_id': pgs_id})

            # Test PRS validation
            if prs_results and prs_results[0]['success']:
                try:
                    validation = calculator.validate_prs_calculation(df, prs_results[0])
                    self.log_success("PRS Validation", f"Coverage: {validation.get('coverage_percentage', 0):.1f}%")
                except Exception as e:
    ...
    def test_clinvar_analysis(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if not os.path.exists(clinvar_path):
                self.log_warning("ClinVar", "ClinVar TSV file not found, skipping analysis")
                return pd.DataFrame()
    ...
    def test_carrier_analysis(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if not carrier_df.empty:
                self.log_success("Carrier Status", f"Analyzed {len(carrier_df)} carrier conditions")
            else:
    ...
    def test_protective_analysis(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if not protective_df.empty:
                self.log_success("Protective Variants", f"Analyzed {len(protective_df)} protective variants")
            else:
                self.log_success("Protective Variants", "No protective variants detected")

            self.results['protective_variants'] = protective_df
            return protective_df
        except Exception as e:
            self.log_error("Protective Variants", e, "Protective variant analysis failed")
            return pd.DataFrame()
    ...
    def test_expanded_carrier_analysis(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if not expanded_carrier_df.empty:
                self.log_success("Expanded Carrier", f"Analyzed {len(expanded_carrier_df)} expanded carrier conditions")
            else:
                self.log_success("Expanded Carrier", "No expanded carrier variants detected")
    ...
    def test_data_loading(self) -> Dict:
        ...
        try:
            ...
            self.log_success("Data Loading", f"Successfully loaded {success_count}/3 datasets")
    ...
    def generate_report(self):
        ...
        # Component status
        components = [
            'DNA Loading', 'API Health', 'Ancestry Inference', 'High Impact Risks',
            'PGx Analysis', 'Wellness Analysis', 'PRS Analysis', 'ClinVar',
            'Carrier Status', 'Protective Variants', 'Expanded Carrier', 'Data Loading'
        ]

        print("COMPONENT STATUS:")
        for component in components:
            status = "PASS" if component.lower().replace(' ', '_') in self.results else "FAIL"
