...
from src.pgx_star_alleles import star_caller, detect_cnv
...
def test_dpyd_alleles():
    ...
    test_cases = [
        ...
        },
        {
            ...
            'expected': {'genotype': '*1/*2A', 'metabolizer': 'Intermediate Metabolizer'}
        },
        {
...
def test_cpic_guidelines_expanded():
    ...
    for drug in cyp2c19_drugs:
        ...
        if drug in recommendations:
            print(f"PASS CYP2C19 Poor Metabolizer - {drug.title()}: {recommendations[drug]}")
        else:
            print(f"FAIL CYP2C19 Poor Metabolizer - {drug.title()}: No recommendation found")
    ...
    for drug in cyp2d6_drugs:
        ...
        if drug in recommendations:
            print(f"PASS CYP2D6 Poor Metabolizer - {drug.title()}: {recommendations[drug]}")
        else:
            print(f"FAIL CYP2D6 Poor Metabolizer - {drug.title()}: No recommendation found")
    ...
    azathioprine_rec = star_caller.get_cpic_recommendations('TPMT', 'Poor', 'azathioprine')
    if 'azathioprine' in azathioprine_rec:
        print(f"PASS TPMT Poor Metabolizer - Azathioprine: {azathioprine_rec['azathioprine']}")
    else:
        print("FAIL TPMT Poor Metabolizer - Azathioprine: No recommendation found")

    # Test DPYD guidelines
    fluorouracil_rec = star_caller.get_cpic_recommendations('DPYD', 'Poor', 'fluorouracil')
    if 'fluorouracil' in fluorouracil_rec:
        print(f"PASS DPYD Poor Metabolizer - Fluorouracil: {fluorouracil_rec['fluorouracil']}")
    else:
...
def run_all_tests():
    ...
    print("Covered medications: clopidogrel, citalopram, codeine, tamoxifen, warfarin, azathioprine, fluorouracil")
