...
def test_ancestry_inference():
    ...
    if ancestry_result.get('success'):
        print(f"Inferred ancestry: {ancestry_result.get('primary_ancestry', 'Unknown')}")
        print(f"Confidence: {ancestry_result.get('confidence', 0.0):.2f}")
        print(f"Admixture proportions: {ancestry_result.get('admixture_proportions', {})}")
...
def test_ancestry_adjusted_prs():
    ...
    if ancestry_result.get('success'):
        ...
        print(f"  Adjustment factor: {adjusted_result[0] / unadjusted_result.get('prs_score', 1):.3f}")
...
def test_validation_functions():
    ...
    print(f"  Ancestry confidence: {ancestry_validation.get('ancestry_confidence', 0):.2f}")
