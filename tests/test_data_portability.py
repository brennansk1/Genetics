...
def create_sample_dna_data():
    ...
    sample_data = {
        'rsID': [
            'rs1801133', 'rs4988235', 'rs4680', 'rs429358', 'rs7412',
            'rs3892097', 'rs4244285', 'rs1057910', 'rs1800462', 'rs1800460'
        ],
        'genotype': [
            'CT', 'TT', 'AG', 'CT', 'CC', 'GG', 'AG', 'CT', 'CT', 'AG'
        ],
        'chromosome': [
            '1', '2', '22', '19', '19', '22', '10', '10', '6', '2'
        ],
        'position': [
            '11856378', '136608646', '19963748', '45411941', '45412079',
            '42522500', '96541615', '96702047', '18138995', '234668879'
        ]
    }
...
def test_csv_export():
    ...
    assert len(lines) == len(dna_data) + 1, f"Expected {len(dna_data) + 1} lines, got {len(lines)}"
...
def test_json_export():
    ...
    assert len(parsed_data) == len(dna_data), f"Expected {len(dna_data)} records, got {len(parsed_data)}"
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        try:
            ...
            if os.path.exists(pdf_path):
                ...
                print(f"SUCCESS: PDF report generated successfully! Size: {file_size} bytes")
                return True
            else:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        try:
            ...
            if os.path.exists(pdf_path):
                ...
                print(f"SUCCESS: Enhanced PDF report generated successfully! Size: {file_size} bytes")
                return True
            else:
...
def test_export_format_consistency():
    ...
    assert 'rsID' in csv_content and 'rsID' in tsv_content, "rsID field missing from CSV/TSV"
    assert all('rsID' in record for record in json_data), "rsID field missing from JSON records"
...
def test_directory_creation():
    ...
    with tempfile.TemporaryDirectory() as temp_base_dir:
        ...
        assert not os.path.exists(results_dir), "Test directory should not exist initially"
