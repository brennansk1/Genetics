...
def add_cover_page(story, user_id):
    ...
    personal_style = ParagraphStyle(
        "Personal", parent=styles["Normal"], fontSize=14, alignment=1, spaceAfter=20
    )
    story.append(Paragraph(f"Prepared for: {user_id}", personal_style))
    story.append(
        Paragraph("Date: " + pd.Timestamp.now().strftime("%B %d, %Y"), personal_style)
    )
    ...
    story.append(
        Paragraph(
            "Transforming genetic data into actionable health insights", tagline_style
        )
    )
...
def add_how_to_read_report(story):
    ...
    analogy_style = ParagraphStyle(
        "Analogy", parent=styles["Normal"], fontSize=12, spaceAfter=20, leftIndent=20
    )
    story.append(
        Paragraph(
            "Some conditions, like Cystic Fibrosis, are strongly influenced by variants in a single gene. Think of it like a <b>light switch</b> - if you have the specific variants, the switch is flipped, and the risk is very high.",
            analogy_style,
        )
    )
    ...
    story.append(
        Paragraph(
            "Most common conditions, like Type 2 Diabetes, are influenced by many genes working together, each having a small effect. Think of it as a <b>dimmer switch</b> - your genetic variants can slide the dimmer up or down, increasing or decreasing your predisposition. Your lifestyle choices are the main hand on the switch.",
            analogy_style,
        )
    )
    ...
    evidence_style = ParagraphStyle(
        "Evidence", parent=styles["Normal"], fontSize=11, spaceAfter=10, leftIndent=20
    )
    story.append(
        Paragraph(
            "Throughout this report, you'll see star ratings that indicate how well-studied and scientifically established a particular gene-trait connection is:",
            evidence_style,
        )
    )
    ...
    story.append(
        Paragraph(
            "Remember: Your genetics load the gun, but your lifestyle pulls the trigger. This report empowers you to make informed choices about your health journey.",
            takeaway_style,
        )
    )
...
def add_genetic_snapshot(story, dna_data):
    ...
    map_desc_style = ParagraphStyle(
        "MapDesc", parent=styles["Normal"], fontSize=10, spaceAfter=15, leftIndent=20
    )
    story.append(
        Paragraph(
            "This constellation map shows your genetic predispositions across different health categories. Larger, brighter stars indicate higher risk with stronger scientific evidence.",
            map_desc_style,
        )
    )
...
def add_detailed_health_chapter(story, condition_name, dna_data, prs_percentile=50):
    ...
    result_style = ParagraphStyle(
        "Result", parent=styles["h2"], fontSize=14, spaceAfter=15
    )
    stars = get_evidence_stars(
        "", "general"
    )  # Would be condition-specific in real implementation
    story.append(
        Paragraph(
            f"Result: Increased Genetic Risk | Strength of Evidence: {stars}",
            result_style,
        )
    )
    ...
    buf = generate_genetics_lifestyle_balance(
        genetics_pct, lifestyle_pct, condition_name
    )
    ...
    bio_title_style = ParagraphStyle(
        "BioTitle",
        parent=styles["h3"],
        fontSize=12,
        spaceAfter=10,
    ...
    bio_style = ParagraphStyle(
        "BioText", parent=styles["Normal"], fontSize=10, spaceAfter=15, leftIndent=20
    )
    ...
    # The PRS Gauge
    prs_title_style = ParagraphStyle(
        "PRSTitle",
        parent=styles["h3"],
        fontSize=12,
        spaceAfter=10,
    ...
    steps_style = ParagraphStyle(
        "StepsText", parent=styles["Normal"], fontSize=10, spaceAfter=8, leftIndent=20
    )
...
def add_medication_guide_section(story, dna_data):
    ...
    intro_style = ParagraphStyle(
        "Intro", parent=styles["Normal"], fontSize=11, spaceAfter=20, leftIndent=20
    )
    story.append(
        Paragraph(
    ...
    for interaction in interactions:
        ...
        funnel_title_style = ParagraphStyle(
            "FunnelTitle",
            parent=styles["h4"],
            fontSize=11,
            spaceAfter=10,
...
def add_comprehensive_carrier_status(story, dna_data):
    ...
    # Import SNP data
    from ..snp_data import ancestry_panels, get_recessive_snps as recessive_snps

    # Combine all carrier conditions
    all_carrier_conditions = recessive_snps()
    ...
    story.append(
        Paragraph(
            f"Analysis Summary: {total_analyzed} conditions analyzed", styles["h3"]
        )
    )
    story.append(
        Paragraph(f"Carrier Status: {carrier_count} conditions", styles["Normal"])
    )
    if affected_count > 0:
        story.append(
            Paragraph(f"Affected Status: {affected_count} conditions", styles["Normal"])
        )
    story.append(Spacer(1, 0.3 * inch))
    ...
    if results:
        ...
        significant_results = [
            r for r in results if r["Status"] in ["Carrier", "Affected"]
        ]
        ...
        if significant_results:
            ...
            data.extend(
                [
                    [r["rsID"], r["Gene"], r["Condition"], r["Genotype"], r["Status"]]
            ...
            table = Table(
                data,
                colWidths=[0.8 * inch, 1.2 * inch, 2 * inch, 0.8 * inch, 1.2 * inch],
            )
            table.setStyle(
                TableStyle(
                    [
            ...
            story.append(
                Paragraph(
                    "No carrier or affected status detected for analyzed conditions.",
                    styles["Normal"],
                )
            )
...
def add_pharmacogenomics_profile(story, dna_data):
    ...
    if results:
        story.append(
            Paragraph(
                f"Analysis Summary: {analyzed_count}/{len(results)} PGx variants analyzed",
                styles["h3"],
            )
        )
        story.append(Spacer(1, 0.3 * inch))
        ...
        if analyzed_results:
            ...
            data.extend(
                [
                    [r["rsID"], r["Gene"], r["Drug"], r["Genotype"], r["Metabolism"]]
            ...
            story.append(
                Paragraph(
                    "No pharmacogenomic variants detected in analyzed data.",
                    styles["Normal"],
                )
            )
    else:
...
def add_disease_risk_assessment(story, dna_data):
    ...
    # Import SNP data
    from ..snp_data import get_cancer_snps as cancer_snps, get_cardiovascular_snps as cardiovascular_snps, get_mito_snps as mito_snps, get_neuro_snps as neuro_snps
    ...
    story.append(
        Paragraph(
            f"Analysis Summary: {total_analyzed} genetic variants analyzed",
            styles["h3"],
        )
    )
    if high_risk_count > 0:
        story.append(
            Paragraph(
                f"High Risk Variants: {high_risk_count} detected", styles["Normal"]
            )
        )
    story.append(Spacer(1, 0.3 * inch))
    ...
    if results:
        ...
        if detected_results:
            ...
            data.extend(
                [
                    [r["rsID"], r["Gene"], r["Condition"], r["Genotype"], r["Risk"]]
            ...
            story.append(
                Paragraph(
                    "No disease-associated variants detected in analyzed regions.",
                    styles["Normal"],
                )
            )
...
def add_polygenic_risk_scores(story, dna_data):
    ...
    # Import SNP data
    from ..snp_data import get_simple_model, get_prs_models as prs_models
    ...
    if analyzed_traits:
        story.append(
            Paragraph(
                f"Analysis Summary: {len(analyzed_traits)} traits analyzed",
                styles["h3"],
            )
        )
        if high_risk_count > 0:
            story.append(
                Paragraph(f"Elevated Risk Traits: {high_risk_count}", styles["Normal"])
            )
        story.append(Spacer(1, 0.3 * inch))
        ...
        if analyzed_traits:
            ...
            story.append(Spacer(1, 0.5 * inch))
            story.append(
                Paragraph(f"PRS Distribution for {first_trait['trait']}:", styles["h3"])
            )
            buf = generate_prs_gauge(
                int(first_trait["percentile"]), first_trait["trait"]
            )
            story.append(Image(buf, width=4 * inch, height=3 * inch))
    else:
    ...
    if wellness_results:
        ...
        for rsid, result in wellness_results.items():
            if result["genotype"] != "Not Found":
                ...
                if (
                    "Vitamin" in trait_name
                    or "Lactose" in trait_name
                    or "Caffeine" in trait_name
                ):
        ...
        for category, traits in categories.items():
            if traits:
                ...
                for rsid, result in traits:
                    ...
                    data.append(
                        [rsid, result["name"], result["gene"], genotype, interpretation]
                    )
        ...
        story.append(
            Paragraph("No wellness SNP data available for analysis.", styles["Normal"])
        )
...
def add_advanced_analytics_dashboard(story, dna_data):
    ...
    from ..snp_data import get_recessive_snps as recessive_snps

    recessive_data = recessive_snps()
    if recessive_data:
        story.append(Paragraph("Carrier Probability Analysis:", styles["h3"]))

        carrier_data = []
        for rsid, info in list(recessive_data.items())[:5]:  # Limit to 5 for display
            carrier_data.append(
                {
                    "Condition": info["condition"],
                    "Status": (
                        "Carrier" if np.random.random() > 0.7 else "Not a carrier"
                    ),  # Sample data
                }
            )
...
def add_methodology_references(story):
    ...
    story.append(
        Paragraph(
            "This report is based on analysis of genetic variants from ClinVar, PharmGKB, and PGS Catalog.",
            styles["Normal"],
        )
    )
...
def add_appendix_raw_data(story, dna_data):
    ...
    if dna_data is not None and not dna_data.empty:
        story.append(
            Paragraph(f"Total SNPs in dataset: {len(dna_data)}", styles["Normal"])
        )
        story.append(Spacer(1, 0.3 * inch))

        # Show first 20 SNPs as sample
        sample_data = (
            dna_data.head(20).reset_index()
            if dna_data.index.name == "rsid"
            else dna_data.head(20)
        )
        ...
        if not sample_data.empty:
            ...
            story.append(
                Paragraph(
                    "Note: This is a sample of the first 20 SNPs. Full dataset contains all analyzed variants.",
                    styles["Italic"],
                )
            )
        else:
            story.append(
                Paragraph("No raw data available to display.", styles["Normal"])
            )
    else:
